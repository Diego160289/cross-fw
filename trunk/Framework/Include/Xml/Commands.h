...
namespace Common
{
  namespace Commands
  {
    ...
    template <XMLDataType>
    struct XMLRetType;

    template <>
    struct XMLRetType<xmlUTF8>
    {
      enum { xmlType = xmlUTF8 };
      typedef std::string Type;
    };

    template <>
    struct XMLRetType<xmlUNICODE>
    {
      enum { xmlType = xmlUNICODE };
      typedef std::wstring Type;
    };

    template <>
    struct XMLRetType<xmlANSI>
    {
      enum { xmlType = xmlANSI };
      typedef std::string Type;
    };

    extern const XMLRetType<xmlUTF8> XmlUtf8;
    extern const XMLRetType<xmlUNICODE> XmlUnicode;
    extern const XMLRetType<xmlANSI> XmlAnsi;

    template <XMLDataType>
    struct XMLConverter;

    template <>
    struct XMLConverter<xmlUNICODE>
    {
      static XMLRetType<xmlUNICODE>::Type
        Convert(const std::wstring &xml, bool useHeader)
      {
        return !useHeader ? xml : std::wstring(L"<?xml version='1.0' encoding='unicode'?>") + xml;
      }
    };

    template <>
    struct XMLConverter<xmlUTF8>
    {
      static XMLRetType<xmlUTF8>::Type
        Convert(const std::wstring &xml, bool useHeader)
      {
        return System::WStringToAString(
          !useHeader ? xml : std::wstring(L"<?xml version='1.0' encoding='utf-8'?>") + xml,
          true);
      }
    };

    template <>
    struct XMLConverter<xmlANSI>
    {
      static XMLRetType<xmlANSI>::Type
        Convert(const std::wstring &xml, bool useHeader)
      {
        return System::WStringToAString(
          !useHeader ? xml : std::wstring(L"<?xml version='1.0' encoding='windows-1251'?>") + xml);
      }
    };

    template <XMLDataType T>
    typename XMLRetType<T>::Type NodeToString(XmlTools::NodePtr node, const XMLRetType<T>&, bool useHeader = false)
    {
      Common::WStringStream XmlStream;
      NodeToXML(node, XmlStream);
      return XMLConverter<static_cast<XMLDataType>(XMLRetType<T>::xmlType)>::Convert(XmlStream.str(), useHeader);
    }
